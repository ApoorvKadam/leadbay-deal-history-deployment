...
Every committed record is synthetic. The generated data uses seed `20260922`. The customer brief and expected-policy contract are hand-authored so the analyzer cannot generate its own answer key.
...
The manifest keeps support, smoothed effect, direction stability, source column, and the exact question text. Ranking follows evidence priority. The hand-authored expected-policy file checks membership separately.

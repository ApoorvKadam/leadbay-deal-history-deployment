...
describe("resolveCasePaths", () => {
  ...
  it("resolves the four runtime input files under the real root", () => {
    const root = makeCase();
    const paths = resolveCasePaths(root);
    expect(Object.values(paths).filter((value) => typeof value === "string")).toHaveLength(5);
  ...
  it("rejects a required-file symlink that resolves outside the case root", () => {
    ...
    for (const name of ["customer-brief.yaml", "historical-deals.csv", "current-prospects.csv"]) {

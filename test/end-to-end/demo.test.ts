...
import { ExpectedPolicySchema } from "../helpers/expected-policy.js";
...
describe("complete synthetic deployment journey", () => {
  it("produces the exact reviewed package and semantic outcomes", async () => {
    ...
    const expected = ExpectedPolicySchema.parse(
      json("fixtures/building-materials-distributor/expected-policy.json"),
    );
    ...
    expect([...selectedIds].sort()).toEqual([...expected.selected_candidate_ids].sort());
    ...
    for (const [id, classification] of Object.entries(expected.rejected_candidates)) {
      expect(evidence.find((item) => item.candidateId === id)?.classification).toBe(classification);
    }
    expect(evaluation.policy_coverage).toBeGreaterThanOrEqual(expected.minimum_policy_coverage);
    expect(evaluation.top_bucket_lift_pp).toBeGreaterThanOrEqual(
      expected.minimum_top_bucket_lift_pp,
    );

...
All customer, deal, and prospect data in this repository is synthetic data. The checked-in `expected-policy.json` file is a test oracle only. The CLI does not load it or use it to select, gate, or score a policy.

The fixture generator deliberately plants known signal and decoy structure. The synthetic lift below is therefore a recovery check for the pipeline, not evidence of customer performance or a claim that the method discovered a real 35-point improvement.

The evidence layer measures customer-supplied CRM or enrichment fields. This demo does not validate Leadbay's public-text answers to the resulting questions. A real deployment would qualify the historical companies through Leadbay, compare Leadbay's own per-question responses with won/lost outcomes, and review that agreement before changing the account.
...
The package exposes `leadbay-deploy` as its bin name. In a source checkout, the reproducible form is the compiled entry point that CI tests:
...
- Biome lint
- strict TypeScript
- **35 test files and 135 passing tests**

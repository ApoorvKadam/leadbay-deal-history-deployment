...
import {
  ...
  type SyntheticQualityGateInput,
...
export interface CommandDependencies {
  ...
  qualityGate: (input: SyntheticQualityGateInput) => void;
...
const DEFAULT_DEPENDENCIES: CommandDependencies = {
  ...
  qualityGate: assertSyntheticQualityGates,
...
function evidenceGate(bundle: AnalysisBundle, deps: CommandDependencies): void {
  ...
  deps.qualityGate({
    ...
    expected: bundle.loaded.expectedPolicy,
  });
}
...
function verifyManifestSource(manifest: PolicyManifest, bundle: AnalysisBundle): void {
  if (
    ...
    manifest.source_fingerprint !== bundle.manifest.source_fingerprint ||
    manifest.leadbay_state.existing_question_count !==
      bundle.manifest.leadbay_state.existing_question_count ||
    manifest.leadbay_state.free_question_slots !== bundle.manifest.leadbay_state.free_question_slots
  ) {
    throw new AppError({
      code: "MANIFEST_SOURCE_MISMATCH",
      exitCode: 2,
      message:
        "The supplied manifest does not match the current case fingerprint or Leadbay state.",
      hint: "Regenerate the manifest from the unchanged case before previewing.",
      details: {
        expected_fingerprint: bundle.manifest.source_fingerprint,
...
export async function runPreview(input: PreviewCommandInput): Promise<ArtifactCommandResult> {
  ...
  if (bundle.selection.deploymentBlocked) evidenceGate(bundle, deps);
  let writer: ArtifactWriter | undefined;

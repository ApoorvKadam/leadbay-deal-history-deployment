...
export const CustomerBriefSchema = schema<CustomerBrief>("CustomerBrief", (input) => {
  ...
  for (const rule of [...candidates, ...vetoes]) {
    ...
    ids.add(rule.id);
    if (prohibitedKeys.has(rule.source_column.trim().toLowerCase())) {
...
export interface ExpectedPolicy {
  ...
  minimum_policy_coverage: number;
  minimum_top_bucket_lift_pp: number;
  maximum_unacknowledged_vetoed_wins: number;
}

export const ExpectedPolicySchema = schema<ExpectedPolicy>("ExpectedPolicy", (input) => {
  const root = record(input, "ExpectedPolicy");
  if (root.schema_version !== "1.0") throw new Error("ExpectedPolicy schema_version must be 1.0.");
  return {
    schema_version: "1.0",
    case_id: string(root.case_id, "case_id"),
    selected_candidate_ids: stringArray(root.selected_candidate_ids, "selected_candidate_ids"),
    exact_questions: Object.fromEntries(
      Object.entries(record(root.exact_questions, "exact_questions")).map(([key, value]) => [
        key,
        string(value, `exact_questions.${key}`),
      ]),
    ),
    anti_pattern_ids: stringArray(root.anti_pattern_ids, "anti_pattern_ids"),
    rejected_candidates: Object.fromEntries(
      Object.entries(record(root.rejected_candidates, "rejected_candidates")).map(
        ([key, value]) => [key, string(value, `rejected_candidates.${key}`)],
      ),
    ),
    minimum_policy_coverage: finiteNumber(root.minimum_policy_coverage, "minimum_policy_coverage"),
    minimum_top_bucket_lift_pp: finiteNumber(
      root.minimum_top_bucket_lift_pp,
      "minimum_top_bucket_lift_pp",
    ),
    maximum_unacknowledged_vetoed_wins: integer(
      root.maximum_unacknowledged_vetoed_wins,
      "maximum_unacknowledged_vetoed_wins",
    ),
  };
});
